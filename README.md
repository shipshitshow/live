...
YOUTUBE_REFRESH_TOKEN_MAIN=...
YOUTUBE_CHANNEL_ID_CLIPS=...
YOUTUBE_REFRESH_TOKEN_CLIPS=...
OPENAI_API_KEY=...
...
- `/comments` pulls the latest YouTube comments across configured channels.
- AI draft replies use `OPENAI_API_KEY`.
- Posting replies requires a YouTube refresh token created with the `youtube.force-ssl` scope.
- If your current refresh token was created with `youtube.readonly`, rerun `bun scripts/youtube-auth.ts` and replace the stored refresh token.

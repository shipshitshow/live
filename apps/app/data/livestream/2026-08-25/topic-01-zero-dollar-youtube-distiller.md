---
title: "[LIVE] Never Watch A YouTube Video Again. We Built It For $0."
...
thumbnail_prompt: "16:9 YouTube livestream thumbnail, 1920x1080, photoreal cinematic render, ultra sharp, soft editorial lighting. PALETTE: warm parchment cream, muted beige, ivory, soft brown shadows, natural skin tones, brushed silver, restrained navy and black; no neon, no cyberpunk, no green money tint. COMPOSITION: two identity-locked hosts large chest-up and cropped by the left and right edges, each roughly 32-35% of frame, framing a centered funnel workflow. CENTER ASSET: an oversized symbolic play-button rectangle at the top rendered in brushed silver, a clean dark-brown funnel beneath it, and three small tidy vertical cards emerging at the bottom; beside the funnel a large embossed parchment price tag hangs from a thin cord showing a single clean zero glyph with a currency stroke through it; a faint dotted outline of a small desktop computer sits behind the hosts pushed back and greyed out like a discarded option; all UI is symbolic with no readable copy. HOST LEFT: preserve Vincent from the injected reference photo, bald with light tan olive skin, stubble, green-hazel eyes, black hoodie, wry knowing expression, one hand lightly dismissing the greyed-out computer. HOST RIGHT: preserve Mitchell from the injected reference photo, dark slicked-back hair, fair skin, blue eyes, navy polo, amused raised-eyebrow expression, subtle pointing gesture at the price tag. BACKGROUND: warm parchment editorial texture with soft beige vignette and faint natural-history sketches of funnels, lenses, and pulleys. CONTRAST RULE: hosts, funnel, and the zero price tag are large and instantly readable at mobile size. LIGHTING: warm frontal studio key light, natural skin, gentle silver rim light on the play-button rectangle. BRANDING: top-right episode number '#27' in muted dark brown with safe margin. TEXT: no title, captions, slogans, model names, brand names, tiny UI labels, or any text except the single zero-with-currency-stroke on the price tag and '#27'. STYLE: premium natural-history tech poster, clean Ship Shit Show live-thumbnail composition. NEGATIVE: no dollar bills, no coins, no cash imagery, no fake logos, no YouTube branding, no generic robots, no dark terminal walls, no clutter, no distorted hands, no watermarks."
---
...
- The shortlist and why each is on it: a code-specific model, a strong general model at 256K, a 550B model at 1M context, and a multimodal 1M model in case the build needs eyes later.
...
- The trigger for switching is not "the output is bad." It is specific: two consecutive failures on the same step, or a loop with no file written.
...
- Clip line: **"You do not pay for free models with money. You pay in turns."**
- Transition: rules set, model picked, ladder declared — it either builds or we watch it fail on free tokens.

### Host Notes

- Pull up: `opencode models` filtered to free routes. 18 of them are routable right here.
- Say the model id out loud when you set it. Chat will hold us to it.
- Don't pretend: we tested all 21. We shortlisted on context, modality, and stated purpose.
...
- **7 · Free model:** review its own work. **Still no paid model.**
- **8 · Vincent:** run it on the Composer 2.5 stream, then on a video from chat.
- **9 · Free model:** stretch — burned subtitles, or the audio-input path with no transcript step.
- **10 · Everyone:** no keys, no private repositories, no client work on camera.
...
| Route | Context | Input | Job |
|---|---|---|---|
| `z-ai/glm-5.2:free` | 256,000 | text | **Primary — writes the code** |
| `cohere/north-mini-code:free` | 256,000 | text | Code fallback |
| `nvidia/nemotron-3-ultra-550b-a55b:free` | 1,000,000 | text | Heavy fallback |
| `minimax/minimax-m3:free` | 1,048,576 | text+image+video | **Runtime distiller — whole transcript, one call** |
| `thinkingmachines/inkling:free` | 1,048,576 | text+image+audio | Stretch — audio in, no transcript step |
| `nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free` | 256,000 | text+image+audio+video | Stretch — omni |
| `google/gemma-4-31b-it:free` | 262,144 | text+image+video | Vision fallback |
| `openrouter/free` | 200,000 | text+image | Auto-router across free routes |
...
**Switch trigger:** two consecutive failures on the same step, or a loop of 90 seconds with no file written. Not "the output looks weak."
...
src/cli.ts          one command, one link
...
- **1 · The $10 asterisk · Vincent · 6 min:** rate limits, confirm our tier live.
- **2 · Architecture · Vincent and Mitchell · 6 min:** why this fits in one request.
- **3 · Model pick · Vincent · 4 min:** set the `:free` route on camera, declare the ladder.
- **4 · Plan · free model · 8–10 min.**
- **5 · Build transcript step · free model · 12–15 min.**
- **6 · Build distill step · free model · 15–20 min.**
- **7 · Build clip step · free model · 10–12 min.**
- **8 · Run it · Vincent · 10 min:** our stream, then a video from chat.
- **9 · Ship it · Vincent · 3 min:** repository public, link in chat.
- **10 · Stretch · free model · remaining:** subtitles or the audio-input path.
...
We are building a small standalone TypeScript CLI called a YouTube distiller. One command, one YouTube link, and the user gets back what the video actually said plus the handful of moments worth watching.
...
G) MVP cut line for a 90-minute live build, and what is explicitly deferred.
...
- runs fetch -> transcript -> distill -> clips
...
## Copy Paste Prompt — P5 Self Review
...
## Copy Paste Prompt — P6 Stretch, Audio Input With No Transcript
...
- Free model list on screen, 21 of them.
...
### B · Run It On Our Own Stream

```bash
bun run src/cli.ts "https://www.youtube.com/watch?v=QoQjddWCnKA" --clips 4
```

- Read the brief out loud. Ask Mitchell whether that is actually what the video was about.
- Open the cut clips and play the top one.
- Point at the request counter. It says 1.

### C · Run It On A Video From Chat

- Take a link from chat, unrehearsed.
- Same command, new URL.

...
contextBridge.exposeInMainWorld('electronAPI', {
  terminal: {
    ...
    spawn: (cols: number, rows: number) => ipcRenderer.invoke('terminal:spawn', { cols, rows }),
  ...
  topics: {
    ...
    read: (date: string, fileName: string) => ipcRenderer.invoke('topics:read', { date, fileName }),
  },
});

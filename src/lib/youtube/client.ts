...
export async function fetchChannelVideos(
  ...
  return cachedFetch(`videos:${channelId}`, TTL.VIDEO_LIST, async () => {
    ...
    const searchRes = await ytFetch<{ items?: YouTubeSearchItem[] }>(searchUrl, token);
    ...
    const videosRes = await ytFetch<{ items?: YouTubeVideoItem[] }>(videosUrl, token);
...
export async function fetchDailyMetrics(
  ...
  return cachedFetch(cacheKey, TTL.DAILY_METRICS, async () => {
    ...
    const res = await ytFetch<YouTubeAnalyticsResponse>(`${ANALYTICS_API}?${params}`, token);

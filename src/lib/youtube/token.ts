...
export function getConfiguredChannelMeta(): Array<{ id: string; label: string; envRefreshToken?: string }> {
  const channels: Array<{ id: string; label: string; envRefreshToken?: string }> = [];
...
async function refreshAccessToken(refreshToken: string): Promise<string> {
  ...
  if (!clientId || !clientSecret) {
    throw new YouTubeAuthError("missing_credentials", "Missing YOUTUBE_CLIENT_ID or YOUTUBE_CLIENT_SECRET");
  }
  ...
  if (!res.ok) {
    ...
    if (res.status === 400 && body.includes("invalid_grant")) {
      throw new YouTubeAuthError("reauth_required", `Token refresh failed: ${res.status} ${body}`);
    }
...
export async function saveRefreshToken(label: string, refreshToken: string) {
  ...
  const meta = getConfiguredChannelMeta().find((channel) => channel.label === label);
  if (meta) {

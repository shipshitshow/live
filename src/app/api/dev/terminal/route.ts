...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Terminal session not found" },
      { status: 404 }
    );
  }
}
...
export async function POST(request: Request) {
  ...
  if (body.action === "exec") {
    ...
    try {
      const snapshot = await getDevTerminalManager().executeCommand(body.sessionId, body.input);
      return NextResponse.json(snapshot, {
        headers: { "Cache-Control": "no-store" },
      });
    } catch (error) {
  ...
  const session = getDevTerminalManager().createSession();

...
## Session 2: Livestream Announcement Tweet Wiring
...
```mermaid
flowchart TD
    A[Open livestream topic request] --> B[Read topic frontmatter from markdown/blob]
    B --> C[Parse announcement_tweet field into Topic]
    C --> D[Return topic via livestream API/store]
    D --> E[Build announcement tweet link on topic page]
    E --> F[Render sidebar CTA and include in key links]
```
...
|-------|------------|
| Content | `data/livestream/2026-04-15/topic-06-ai-backlash-doesnt-matter.md` |
| Data model | `src/lib/livestream-types.ts` |
| Storage/parsing | `src/lib/livestream-store.ts` |
| Frontend | `src/app/livestream/[slug]/page.tsx` |

### What was done

- [x] Added `announcement_tweet` frontmatter to the `ai-backlash-doesnt-matter` topic
- [x] Extended the livestream topic frontmatter type to carry `announcement_tweet`
- [x] Updated markdown parsing and serialization so the field survives filesystem/blob round-trips
- [x] Added a dedicated `Open Announcement Tweet` CTA to the livestream topic sidebar
- [x] Included the announcement tweet in the page’s key links stack
- [x] Verified the topic API now returns the new tweet URL for this slug
- [x] Ran `bun run check:types`

### Key decisions

- **Decision:** Model the tweet as a general frontmatter field instead of hardcoding the `ai-backlash-doesnt-matter` slug in the page
  - **Context:** The request was for one livestream page, but the page already uses topic metadata for other stream-specific actions
  - **Rationale:** This keeps future announcement tweets data-driven and avoids one-off component logic

- **Decision:** Surface the tweet both as a primary sidebar action and as a key link
  - **Context:** The page already has a CTA block plus a compact link summary
  - **Rationale:** The stream host gets one obvious button while still keeping the tweet in the reusable link list

- **Decision:** Normalize missing `announcement_tweet` values to `null`
  - **Context:** Older topics do not have this field yet
  - **Rationale:** Prevents accidental `"undefined"` serialization when existing topics are re-saved

### Files changed

- `data/livestream/2026-04-15/topic-06-ai-backlash-doesnt-matter.md` - added the announcement tweet URL to frontmatter
- `src/lib/livestream-types.ts` - added `announcement_tweet` to topic frontmatter
- `src/lib/livestream-store.ts` - parsed and serialized `announcement_tweet`, defaulting absent values to `null`
- `src/app/livestream/[slug]/page.tsx` - rendered the announcement tweet action and added it to key links
- `.agents/SESSIONS/2026-04-15.md` - appended this session entry

### Mistakes and fixes

- Tried to `curl` the local livestream page without quoting the query string in `zsh`
  - Quoted the URL and re-ran the check

- Direct HTML grep against the page response was not a reliable verification for the rendered CTA
  - Verified the underlying data path through `/api/livestream?date=2026-04-15` and confirmed the slug returns the expected `announcement_tweet`

### Verification

- `bun run check:types`
- `curl -s 'http://localhost:3001/api/livestream?date=2026-04-15'`
  - Confirmed `ai-backlash-doesnt-matter` returns `announcement_tweet: "https://x.com/shipshitdev/status/2044433544559091936?s=20"`

### Next steps

- [ ] If more livestream topics need this, add `announcement_tweet` frontmatter where relevant
- [ ] If the page should show an actual embed instead of a link, add a dedicated tweet/embed card rather than overloading the key-links list
